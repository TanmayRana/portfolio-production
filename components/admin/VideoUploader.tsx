import { Film, Loader2, Eye, Trash2 } from "lucide-react";
...
export default function VideoUploader({ label = "Video", currentUrl, onUpload }: VideoUploaderProps) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      try {
        ...
        const result = await uploadToImageKit(file, "/videoProtfolio/video");
  ...
  return (
    ...
    >
      ...
      <div className="text-sm font-medium">
        {isUploading ? "Uploading..." : currentUrl ? "Click to change video" : `Drag & drop your ${label} here`}
      </div>
      <div className="text-xs text-muted-foreground">
        {isUploading ? "Please wait" : "Or click to select an MP4/WebM file"}
      </div>
    </div>
  );
}
